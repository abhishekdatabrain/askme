    ...
    <div className={`min-h-screen font-sans flex transition-colors duration-200 ${theme === 'light' ? 'bg-[#F4F5F7] text-[#1A1D20] selection:bg-[#00F5D4] selection:text-[#0A0A0F]' : 'bg-[#0A0A0F] text-[#F5F5F7] selection:bg-[#00F5D4] selection:text-[#0A0A0F]'
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="p-6 max-w-6xl w-full mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {/* Pending Withdrawal */}
            <div className={`p-5 rounded-3xl border space-y-2 shadow-xl transition-colors duration-200 ${theme === 'light' ? 'bg-white border-[#E9ECEF]' : 'bg-[#13131A] border-[#1C1C26]'
              }`}>
            ...
            {activeTab === 'history' && (
              <div>
                {isLoading ? (
                  ...
                  <div className={`p-8 rounded-2xl border text-center space-y-3 ${theme === 'light' ? 'bg-[#F8F9FA] border-[#E9ECEF]' : 'bg-[#0A0A0F] border-[#1C1C26]'
                    }`}>
                        ...
                        {withdrawals.map((w) => (
                          <tr key={w.id} className={`transition ${theme === 'light' ? 'hover:bg-[#F8F9FA]' : 'hover:bg-[#1A1A26]/50'
                            ...
                            <td className="py-3.5 px-3 text-right whitespace-nowrap">
                              <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase inline-flex items-center gap-1 ${w.status === 'completed' || w.status === 'Approved'
      ...
      {showWithdrawModal && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 p-4 flex items-center justify-center">
          <div className={`border rounded-3xl w-full max-w-lg p-6 space-y-5 shadow-2xl animate-scale-up ${theme === 'light' ? 'bg-white border-[#E9ECEF]' : 'bg-[#13131A] border-[#1C1C26]'
            }`}>
            <div className={`flex items-center justify-between border-b pb-4 ${theme === 'light' ? 'border-[#E9ECEF]' : 'border-[#1C1C26]'
              ...
              <button
                ...
                className={`p-1 rounded-xl transition ${theme === 'light' ? 'bg-[#F1F3F5] text-[#6C757D] hover:text-[#1A1D20]' : 'bg-[#1C1C26] text-[#8B8B96] hover:text-white'
                  }`}
              >
            ...
            <form onSubmit={handleWithdrawSubmit} className="space-y-4">
              <div className={`p-3.5 rounded-2xl border flex items-center justify-between ${theme === 'light' ? 'bg-[#F8F9FA] border-[#E9ECEF]' : 'bg-[#0A0A0F] border-[#1C1C26]'
                }`}>
              ...
              <div className={`flex items-center justify-end gap-3 pt-2 border-t ${theme === 'light' ? 'border-[#E9ECEF]' : 'border-[#1C1C26]'
                }`}>
                <button
                  type="button"
                  onClick={() => setShowWithdrawModal(false)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold ${theme === 'light' ? 'bg-[#E9ECEF] text-[#1A1D20] hover:bg-[#DEE2E6]' : 'bg-[#1C1C26] text-white hover:bg-[#252533]'
                    }`}
                >
                  Cancel
                </button>
