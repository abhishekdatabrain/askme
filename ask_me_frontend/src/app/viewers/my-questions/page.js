    ...
    <>
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="p-6 max-w-5xl w-full mx-auto space-y-6">
          ...
          <div className={`p-6 rounded-3xl border space-y-4 shadow-xl ${theme === 'light' ? 'bg-white border-[#E2E8F0]' : 'bg-[#12121C] border-[#1F1F30]'
            }`}>

            {/* QUESTIONS LIST */}
            <div className="pt-2 space-y-4">
              <h3 className={`font-heading font-extrabold text-base flex items-center gap-2 ${theme === 'light' ? 'text-[#0F172A]' : 'text-white'}`}>
                <Radio className="h-5 w-5 text-[#EB1000]" />
                Live Broadcast Question History
              </h3>

              {isLoading ? (
                <div className="p-12 text-center text-xs text-[#8B8B96] space-y-2">
                  <RefreshCw className="h-8 w-8 border-2 border-[#EB1000] border-t-transparent rounded-full animate-spin mx-auto text-[#EB1000]" />
                  <p>Loading your questions history...</p>
                </div>
              ) : questions.length === 0 ? (
                <div className={`p-12 rounded-2xl border text-center space-y-3 ${theme === 'light' ? 'bg-[#F8FAFC] border-[#E2E8F0]' : 'bg-[#0D0D14] border-[#1F1F30]'
                  }`}>
                  <MessageSquare className="h-10 w-10 mx-auto text-[#8B8B96] opacity-40" />
                  <h4 className={`font-bold text-sm ${theme === 'light' ? 'text-[#0F172A]' : 'text-white'}`}>No Questions Asked Yet</h4>
                  <p className={`text-xs max-w-md mx-auto ${theme === 'light' ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
                ...
                questions.map((q, idx) => (
                  <div
                    ...
                    className={`p-5 rounded-2xl border space-y-3 shadow-md transition ${q.isVip
                      ? 'bg-[#1C1805] border-2 border-amber-500/80 shadow-xl glow-gold'
                      : theme === 'light'
                        ? 'bg-[#F8FAFC] border-[#E2E8F0]'
                        : 'bg-[#0D0D14] border-[#1F1F30]'
                      }`}
                  >
                    <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-3 ${theme === 'light' ? 'border-[#E2E8F0]' : 'border-[#1F1F30]'}`}>
                      <div className="flex items-center gap-3">
                        <img
                          src={q.creatorAvatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=400&q=80'}
                          alt={q.creatorName}
                          className="h-10 w-10 rounded-full border border-[#EB1000]/40 object-cover shrink-0"
                        />
                        <div>
                          <h4 className={`font-bold text-sm flex items-center gap-1.5 ${theme === 'light' ? 'text-[#0F172A]' : 'text-white'}`}>
                            {q.creatorName}
                          </h4>
                          <p className={`text-xs flex items-center gap-1 ${theme === 'light' ? 'text-[#64748B]' : 'text-[#94A3B8]'}`}>
                            <Tv className="h-3 w-3 text-[#EB1000]" /> {q.sessionTitle}
                          </p>
                        </div>

                        {q.isVip && (
                          <span className="px-2.5 py-0.5 rounded-full bg-gradient-to-r from-amber-400 to-amber-600 text-white text-[10px] font-black uppercase flex items-center gap-1 shadow-md ml-2">
                            ⚡ VIP PRIORITY
                          </span>
                        )}
                      </div>
                    ...
                    {q.message && (
                      <div className="pt-1">
                        ...
                        <p className={`p-3 rounded-2xl text-xs italic border font-medium ${q.isVip
                          ? 'bg-[#0D0D14] text-amber-400 border-amber-500/40'
                          : theme === 'light'
                            ? 'bg-white border-[#E2E8F0] text-[#EB1000]'
                            : 'bg-[#14141F] text-[#EB1000] border-[#1F1F30]'
                          }`}>
                    ...
                    <div className="flex items-center justify-between pt-1 text-xs">
                      <span className="text-xs text-[#8B8B96] font-medium flex items-center gap-1">
                        <Clock className="h-3.5 w-3.5 text-[#EB1000]" />
