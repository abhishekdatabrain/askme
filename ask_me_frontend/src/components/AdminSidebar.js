...
export default function AdminSidebar({ activeTab, setActiveTab, activeSubTab, setActiveSubTab, theme = 'dark', onToggleTheme }) {
  ...
  return (
    <aside className={`w-64 shrink-0 border-r p-4 flex flex-col justify-between hidden md:flex sticky top-[61px] h-[calc(100vh-61px)] self-start overflow-y-auto custom-scrollbar transition-colors ${theme === 'light'
      ...
      <div className="space-y-4">
        {/* Section Header */}
        <div>
          <span className={`text-[11px] font-bold tracking-wider uppercase px-3 ${theme === 'light' ? 'text-[#6C757D]' : 'text-[#8B8B96]'
            }`}>
            Admin Navigation
          </span>
        ...
        <div className={`p-3.5 rounded-2xl border space-y-3 ${theme === 'light' ? 'bg-[#F8F9FA] border-[#E9ECEF]' : 'bg-[#13131A] border-[#1C1C26]'
          }`}>
