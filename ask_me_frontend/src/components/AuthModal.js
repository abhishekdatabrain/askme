    ...
    try {
      if (role === 'viewer') {
        ...
        if (mode === 'login') {
          ...
          if (!fname || !lname || !email || !cleanMobile || !password || !username) {
            const errText = 'Please fill in all required personal details for Creator registration.';
            toast?.error(errText, 'Registration Error');
            throw new Error(errText);
          }
          if (cleanMobile.length !== 10) {
        ...
        <div className="px-6 py-5 overflow-y-auto space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div>
                  <h4 className="font-bold text-xs text-gray-900">Creator Pro</h4>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            {role === 'creator' && mode === 'register' ? (
              <div className="space-y-4">
                {/* Step Indicator Bar */}
                <div className="flex items-center justify-between gap-2 px-3.5 py-2 rounded-2xl bg-gray-50 border border-gray-100">
                  <button
                    type="button"
                    onClick={() => setCreatorStep(1)}
                    className="flex items-center gap-2 focus:outline-none cursor-pointer"
                  >
                    <span className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold ${creatorStep >= 1 ? 'bg-[#EB1000] text-white' : 'bg-gray-200 text-gray-500'}`}>
                      1
                    </span>
                    <span className={`text-xs font-bold ${creatorStep === 1 ? 'text-gray-900' : 'text-gray-400'}`}>
                      Personal Details
                    </span>
                  </button>

                  <span className="h-0.5 w-8 bg-gray-200"></span>

                  <button
                    type="button"
                    onClick={handleNextCreatorStep}
                    className="flex items-center gap-2 focus:outline-none cursor-pointer"
                  >
                    <span className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold ${creatorStep >= 2 ? 'bg-[#EB1000] text-white' : 'bg-gray-200 text-gray-500'}`}>
                      2
                    </span>
                    <span className={`text-xs font-bold ${creatorStep === 2 ? 'text-gray-900' : 'text-gray-400'}`}>
                      Social Media Links
                    </span>
                  </button>
                </div>
                  ...
                  <div className="space-y-3.5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="relative">
                          <AtSign className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-[#EB1000]" />
                          <input
                            type="text"
                            required
                            value={username}
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            {getUsernameSuggestions().map((sug) => (
                              <button
                                ...
                                className="px-1.5 py-0.5 rounded bg-rose-50 text-[#EB1000] text-[10px] font-mono hover:bg-rose-100 transition cursor-pointer"
                              >
                                @{sug}
                              </button>
                            ))}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          className="w-full pl-10 pr-4 py-2.5 rounded-2xl border border-gray-200 text-xs bg-gray-50/50 text-gray-900 focus:bg-white focus:border-[#EB1000] outline-none transition"
                        >
                          {countriesList.map((c) => (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 space-y-1">
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-xl border border-gray-200 text-xs bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:bg-white focus:border-[#EB1000] outline-none transition"
                          />
                    ...
                    <button
                      type="button"
                      onClick={handleNextCreatorStep}
                      className="w-full py-3 px-4 rounded-2xl bg-gray-900 hover:bg-black text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-2 cursor-pointer mt-2"
                    >
                      <span>Next: Social Media Links</span>
                      <ArrowRight className="h-4 w-4" />
                    </button>
                  </div>
                ) : (
                  <div className="space-y-3.5">
                    <label className="block text-xs font-bold text-gray-700">Social Media Links</label>
                    {socialLinks.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        <select
                          ...
                          className="py-2 px-2.5 rounded-xl border border-gray-200 text-xs bg-gray-50 text-gray-900 outline-none"
                        >
                        ...
                        <input
                          ...
                          className="flex-1 py-2 px-3 rounded-xl border border-gray-200 text-xs bg-gray-50 text-gray-900 outline-none"
                        />
                    ...
                    <button
                      type="button"
                      onClick={() => setSocialLinks([...socialLinks, { platform: 'youtube', link: '' }])}
                      className="text-xs font-bold text-[#EB1000] hover:underline cursor-pointer"
                    >
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        type="button"
                        onClick={() => setCreatorStep(1)}
                        className="w-1/3 py-3 rounded-2xl bg-gray-100 text-gray-700 font-bold text-xs hover:bg-gray-200 transition cursor-pointer"
                      >
                        Back
                      </button>
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-2/3 py-3 rounded-2xl bg-[#EB1000] text-white font-bold text-xs shadow-lg shadow-[#EB1000]/25 transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                      >
                        {loading ? 'Registering...' : 'Create Creator Pro Account ➔'}
                      </button>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="email"
                      required
                      placeholder={mode === 'register' ? 'rahul@example.com' : (role === 'viewer' ? 'you@example.com' : 'creator@askme.com')}
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      className="w-full pl-10 pr-4 py-2.5 rounded-2xl border border-gray-200 text-xs bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:bg-white focus:border-[#EB1000] outline-none transition"
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type={showPassword ? 'text' : 'password'}
                      required
                      placeholder={mode === 'register' ? 'e.g. StrongPass@123' : '••••••••'}
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      className="w-full pl-10 pr-10 py-2.5 rounded-2xl border border-gray-200 text-xs bg-gray-50/50 text-gray-900 placeholder-gray-400 focus:bg-white focus:border-[#EB1000] outline-none transition"
                    />
                  ...
                  {loading ? (
                    ...
                    <>
                      <span>
                        {mode === 'login'
                          ? `Sign In as ${role === 'viewer' ? 'Viewer' : 'Creator Pro'}`
                          : `Create ${role === 'viewer' ? 'Viewer' : 'Creator Pro'} Account`}
                      </span>
                      <ArrowRight className="h-4 w-4" />
                    </>
                  )}
          ...
          <div className="space-y-2 pt-1">
            ...
            <TruecallerAuthButton
              label="Truecaller 1-Tap Verify"
              className="rounded-2xl py-2.5"
              isLoading={loading}
              onError={(err) => setErrorMsg(err)}
              onSuccess={(tcData) => handleTruecallerAuth(tcData)}
            />
