...
export default function CreatorSidebar() {
  ...
  useEffect(() => {
    ...
    try {
      const storedUserStr = localStorage.getItem('askme_user');
      if (storedUserStr) {
        ...
        if (data.status === 'success' && data.data?.kycStatus) {
          ...
          if (storedUserStr) {
            try {
              ...
              localStorage.setItem('askme_user', JSON.stringify(parsed));
            } catch (err) {}
          }
        }
      } catch (err) {
                ...
                <div className="ml-1 shrink-0">
                  {kycStatus === 'approved' && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 ${
                      isActive ? 'bg-[#0A0A0F]/20 text-[#0A0A0F]' : 'bg-[#00E676]/10 text-[#00E676] border border-[#00E676]/30'
                    }`}>
                  ...
                  {kycStatus === 'rejected' && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 ${
                      isActive ? 'bg-[#0A0A0F]/20 text-[#0A0A0F]' : 'bg-[#FF3D71]/10 text-[#FF3D71] border border-[#FF3D71]/30'
                    }`}>
                  ...
                  {(kycStatus === 'pending' || (kycStatus !== 'approved' && kycStatus !== 'rejected')) && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 ${
                      isActive ? 'bg-[#0A0A0F]/20 text-[#0A0A0F]' : 'bg-[#FFD60A]/10 text-[#FFD60A] border border-[#FFD60A]/30'
                    }`}>
