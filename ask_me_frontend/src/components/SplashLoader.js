...
export default function SplashLoader({ message = 'Loading AskMe Live Feed...' }) {
  return (
    <div className="fixed inset-0 z-50 bg-[#0A0A0F] text-white flex flex-col items-center justify-center p-6 select-none font-sans animate-in fade-in duration-300">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 rounded-full bg-[#EB1000]/15 blur-3xl pointer-events-none animate-pulse"></div>

      <div className="relative z-10 text-center space-y-6 max-w-sm mx-auto">
        {/* Glowing Logo Icon */}
        <div className="relative inline-block animate-bounce">
          <Logo size="2xl" />
          <div className="absolute -top-1.5 -right-1.5 h-6 w-6 rounded-full bg-[#EB1000] border-2 border-[#0A0A0F] flex items-center justify-center animate-ping">
        ...
        <div className="space-y-1.5">
          <h1 className="font-heading font-black text-2xl tracking-wide text-white">
            AskMe <span className="text-[#EB1000]">STUDIO</span>
          </h1>
          <p className="text-xs font-semibold text-[#EB1000] flex items-center justify-center gap-1.5 tracking-wider uppercase">
        ...
        <div className="space-y-3 pt-2">
          <div className="w-48 h-1 bg-[#1C1C26] rounded-full mx-auto overflow-hidden">
            <div className="h-full bg-[#EB1000] rounded-full animate-pulse w-3/4"></div>
          </div>
